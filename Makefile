...
# beside `bin/iyi` is 540 KB rather than a standard library.
.PHONY: install_iyi
...
# ships only iyi's own 540 KB answers `require "json"` with "can't find file",
...
# Crystal's library requires `std/...`, the directory's 1,889 KB is a second
...
# Its prelude is its own, and it is 540 KB: `iyi` installed as `bin/iyi` finds
...
# iyi: the self-hosted parser syntax check tool, built by iyi from pure iyi source.
