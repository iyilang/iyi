...
| iyi's own prelude | 15,899 lines, of which 3,285 are the library held to the 3,734 ceiling (4,751 with every platform's floor, which the ceiling stopped counting after Windows); the rest is the collector, the scheduler and the float printer, which 0.1.0's prelude got from libgc, pthreads and libc |
| compiler | 113,045 lines, none of it written in iyi |
...
> "No standard library worth the name" is still true of iyi's own 15,899 lines
...
**3. A deliberately tiny prelude, written in iyi. Done: 15,899 lines,
primitives included, of which the library is 3,285.** Not a standard library:
...
**3,285 lines** of the 15,899, measured by `bench/doc_numbers.py` as
everything under `src/iyi/` except those three and except every platform's
floor of 1,466 lines — the arms behind `flag?(:win32)`, `flag?(:linux)`,
`flag?(:darwin)` and `flag?(:wasm32)`, which the paragraph on the breach
below settles and explains. Opening `src/iyi/` counts 4,751 with the floor
...
**And it was breached again by Windows' own floor — 4,751 of 3,734, 1,017
...
where most of the 1,017 lines are.
...
exists for the same reason — the platform floor measures **1,466 lines**
of the 4,751, and Windows is the largest arm of it by a wide margin, which
...
The rewrite means the library gives back 1,017 lines, which at this size
...
and the library is **3,285** of 3,734 with 449 to spare.
...
|---|---|---|
| Compiler | 24,984 lines, **written in Crystal** | 113,045 lines, Crystal, forked |
| Library | 8,161 lines (3,551 of it core) | 15,899-line own prelude + 38,978 in std |
| Specs | 21,146 lines | 11,063 for iyi |
...
12. ~~**Using Crystal code.**~~ **Specified in III.6; the direct source mode
    ...
    **The Crystal ecosystem, and what a shard would cost.** Ten thousand
    ...
    Crystal's 8,161-line standard library against iyi's own 15,899-line prelude.
