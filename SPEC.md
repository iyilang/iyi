...
| iyi's own prelude | 15,637 lines, of which 3,219 are the library held to the 3,734 ceiling (4,581 with every platform's floor, which the ceiling stopped counting after Windows); the rest is the collector, the scheduler and the float printer, which 0.1.0's prelude got from libgc, pthreads and libc |
...
> "No standard library worth the name" is still true of iyi's own 15,637 lines
...
**3. A deliberately tiny prelude, written in iyi. Done: 15,637 lines,
primitives included, of which the library is 3,219.** Not a standard library:
...
**3,219 lines** of the 15,637, measured by `bench/doc_numbers.py` as
everything under `src/iyi/` except those three and except every platform's
floor of 1,362 lines — the arms behind `flag?(:win32)`, `flag?(:linux)`,
`flag?(:darwin)` and `flag?(:wasm32)`, which the paragraph on the breach
below settles and explains. Opening `src/iyi/` counts 4,581 with the floor
...
**And it was breached again by Windows' own floor — 4,581 of 3,734, 847
...
where most of the 847 lines are.
...
exists for the same reason — the platform floor measures **1,362 lines**
of the 4,581, and Windows is the largest arm of it by a wide margin, which
...
The rewrite means the library gives back 847 lines, which at this size
...
and the library is **3,219** of 3,734 with 515 to spare.
...
| Library | 8,161 lines (3,551 of it core) | 15,637-line own prelude + 38,986 in std |
...
12. ~~**Using Crystal code.**~~ **Specified in III.6; the direct source mode
    ...
    **The Crystal ecosystem, and what a shard would cost.** Ten thousand
    ...
    Crystal's 8,161-line standard library against iyi's own 15,637-line prelude.
