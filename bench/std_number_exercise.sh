...
build_and_run() {
  ...
  shift 2
  if ! "$IYI" build "$@" -o "$WORK/$name" "$REPO/bench/std_number_exercise.iyi" \
...
echo "== the std/number exercise, plain build"
build_and_run "plain" number-plain
if ! grep -q "ALL CHECKS PASSED" "$WORK/number-plain.out" 2>/dev/null; then
...
build_and_run "release" number-release --release >/dev/null
if ! grep -q "ALL CHECKS PASSED" "$WORK/number-release.out" 2>/dev/null; then
...
PY
  if [ $? -ne 0 ]; then
    ...
    status=1
  elif IYI_PATH="$WORK/patched${PSEP}$REPO/src${PSEP}$REPO/samples/iyi" "$IYI" run "$REPO/bench/std_number_exercise.iyi" >"$WORK/mut.out" 2>&1; then
    echo "  the exercise PASSED on a broken module"
    status=1
  else
    echo "  a broken number is caught"
  fi
fi

echo
if [ "$status" -eq 0 ]; then
  echo "the std/number exercise holds"
else
  echo "the std/number exercise did not hold"
fi
exit $status
