...
old = 'span = @nfa.run(text, 0, true)'
if old not in src:
    raise SystemExit("patch site missing")
Path("$WORK/patched/std/regex.iyi").write_text(src.replace(old, 'span = {0, 0}', 1))
PY
if [ $? -ne 0 ]; then
  echo "  the patch did not apply"
  status=1
elif IYI_PATH="$WORK/patched:$REPO/src:$REPO/samples/iyi" "$IYI" run "$REPO/bench/std_regex_exercise.iyi" >"$WORK/mut.out" 2>&1; then
  echo "  the exercise PASSED on a broken module"
  status=1
else
  echo "  a broken regex is caught"
fi
