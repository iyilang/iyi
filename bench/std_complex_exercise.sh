#!/usr/bin/env bash
# Exercises `std/complex`: Complex abs, arithmetic, exp.
#
...
for phrase in "== abs" "== arithmetic"; do
...
old = 'Math.hypot(@real, @imag)'
if old not in src:
    raise SystemExit("patch site missing")
Path("$WORK/patched/std/complex.iyi").write_text(src.replace(old, '@real + @imag', 1))
PY
then
  echo "  the patch did not apply"
  status=1
elif IYI_PATH="$WORK/patched${PSEP}$REPO/src${PSEP}$REPO/samples/iyi" "$IYI" run "$REPO/bench/std_complex_exercise.iyi" >"$WORK/mut.out" 2>&1; then
  echo "  the exercise PASSED on a broken module"
  status=1
else
  echo "  a broken complex is caught"
fi
