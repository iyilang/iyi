...
| `tools/bind.iyi` | 1,060 | `bench/selfhost_bind_exercise.sh`: 2 fixtures, 24 public methods, five guarded mutation proofs. Driven against the shipped `Iyi.print_bind` |
...
| `codegen/codegen.iyi` | 1,980 | `bench/selfhost_codegen_exercise.sh`: 11 fixtures, 67 functions, 9 struct and class types, 8 type ID globals, and allocator declarations with 100% identical LLVM IR and identical native object execution linked with C driver, nine guarded mutation proofs. Emits LLVM IR for `fun` declarations with integer and float arithmetic, comparisons, local variable allocation and assignments, `if`/`else` (with phi value merges), `while` loops, and inter-function calls; structs (stack allocation, zero-initialization via memset, field accessors, pass-by-value arguments, methods, initializers, and constructors); pointer operations (pointerof, value, value=, ptr + offset, ptr - ptr, address); classes (heap allocation via malloc, zero-initialization, fields, pass-by-reference, methods, initializers, and constructors); virtual method dispatch (vtable structs, vtable global instances, method indices, interface-like trait dispatch via dispatch thunks, and polymorphic calls); and nilable reference and value types (null pointer checks, tagged pointer representations, and nil literal checks). Still not in the port: closures and blocks, exceptions, and garbage collector integration |
...
check), TypeNode inspection in macros, and codegen beyond fun, struct, class, virtual dispatch, and nilable slices
