...
| `codegen/codegen.iyi` | 2,999 | `bench/selfhost_codegen_exercise.sh`: 14 fixtures, 91 functions, 13 struct, class and closure types, 8 type ID globals, and allocator declarations with 100% identical LLVM IR and identical native object execution linked with C driver, thirteen guarded mutation proofs. Emits LLVM IR for `fun` declarations with integer and float arithmetic, comparisons, local variable allocation and assignments, `if`/`else` (with phi value merges), `while` loops, and inter-function calls; structs (stack allocation, zero-initialization via memset, field accessors, pass-by-value arguments, methods, initializers, and constructors); pointer operations (pointerof, value, value=, ptr + offset, ptr - ptr, address); classes (heap allocation via malloc, class type IDs, field accessors, virtual dispatch, method inheritance, and initializers); block inlining with parameter passing; exception handling (raise, try/rescue/ensure, landing pads, invoke, and cleanup); and proc closures with captured environments and indirect calls via trampoline |
...
| `tools/compile.iyi` | 100 | `bench/selfhost_compile_exercise.sh`: 9 fixtures (including multi-file import and diamond dependency), 100% execution parity and dependency floor against the shipped compiler, refusal parity on missing imports |
...
virtual dispatch, nilable, block inlining, exception, and proc closure slices (generics,
and the GC interface). That is the
