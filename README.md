...
it uses, and iyi's own library is 17,489 lines rather than 8,161. The whole
library is 720 KB on disk beside the binary.
...
iyi's own 720 KB prelude, the 1,339 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 17,489
...
- **iyi's own library is 17,489 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 17,489 lines. `--crystal` swaps it for Crystal's |
