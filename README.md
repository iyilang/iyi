...
current compiler reports `0.15.2`. iyi's own prelude carries the small-tool
...
it uses, and iyi's own library is 17,838 lines rather than 8,161. The whole
library is 736 KB on disk beside the binary.
...
The released tarball is 0.15.2, and a build from current source reports the
...
`IYI_PREFIX` moves the destination and `IYI_VERSION=0.15.2` pins a
release. By hand it is the same two lines:

```sh
tar -xzf iyi-0.15.2-linux-x86_64.tar.gz -C ~/.local
~/.local/bin/iyi run ~/.local/share/iyi/samples/hello.iyi
```

The tarball is relocatable and carries every library a program can ask for:
iyi's own 736 KB prelude, the 1,338 KB of `src/std` that `import std/...`
...
`iyi-0.15.2-windows-x86_64.zip` sits beside the two tarballs,
...
```powershell
Expand-Archive iyi-0.15.2-windows-x86_64.zip -DestinationPath "$env:LOCALAPPDATA\Programs\iyi"
& "$env:LOCALAPPDATA\Programs\iyi\bin\iyi.exe" run "$env:LOCALAPPDATA\Programs\iyi\share\iyi\samples\hello.iyi"
```
...
program *has*: 8,161 lines of Crystal's standard library instead of 17,838
...
**Is the syntax stable?** No. 0.15.2 is a release of a language that is still
...
language rather than the library — master is 0.15.2, and the parts of SPEC.md
marked PROPOSED are the parts that will move under you.

## What is not here

- **iyi's own library is 17,838 lines, and its IO is `puts`, `print`, the
...
- **Artifacts are identified by released version, target and flag set.** Builds
  ...
  commit in its identity and interoperates only with itself. 0.15.2 is
  released, so its artifacts carry the version alone and any 0.15.2 build on
...
| [`src/iyi`](src/iyi) | iyi's own library, 17,838 lines. `--crystal` swaps it for Crystal's |
...
`iyi 0.15.2 (built on Crystal 1.22.0-dev)`: the language first, then what it
