...
# beside `bin/iyi` is 548 KB rather than a standard library.
.PHONY: install_iyi
...
# ships only iyi's own 548 KB answers `require "json"` with "can't find file",
...
# Crystal's library requires `std/...`, the directory's 1,137 KB is a second
...
# Its prelude is its own, and it is 548 KB: `iyi` installed as `bin/iyi` finds
