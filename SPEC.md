...
| iyi's own prelude | 17,863 lines, of which 3,509 are the library held to the 3,734 ceiling (5,049 with every platform's floor, which the ceiling stopped counting after Windows); the rest is the collector, the scheduler and the float printer, which 0.1.0's prelude got from libgc, pthreads and libc |
...
> "No standard library worth the name" is still true of iyi's own 17,863 lines
...
**3. A deliberately tiny prelude, written in iyi. Done: 17,863 lines,
...
floor of 1,540 lines — the arms behind `flag?(:win32)`, `flag?(:linux)`,
`flag?(:darwin)` and `flag?(:wasm32)`, which the paragraph on the breach
below settles and explains. Opening `src/iyi/` counts 5,049 with the floor
...
exists for the same reason — the platform floor measures **1,540 lines**
...
| Library | 8,161 lines (3,551 of it core) | 17,863-line own prelude + 40,654 in std |
...
12. ~~**Using Crystal code.**~~ **Specified in III.6; the direct source mode
    ...
    **The Crystal ecosystem, and what a shard would cost.** Ten thousand
    ...
    Crystal's 8,161-line standard library against iyi's own 17,863-line prelude.
