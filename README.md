...
it uses, and iyi's own library is 14,216 lines rather than 8,161. The whole
...
iyi's own 551 KB prelude, the 1,154 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 14,216
...
- **iyi's own library is 14,216 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 14,216 lines. `--crystal` swaps it for Crystal's |
