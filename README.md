...
it uses, and iyi's own library is 14,135 lines rather than 8,161. The whole
library is 548 KB on disk beside the binary.
...
iyi's own 548 KB prelude, the 1,137 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 14,135
...
- **iyi's own library is 14,135 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 14,135 lines. `--crystal` swaps it for Crystal's |
