...
it uses, and iyi's own library is 17,219 lines rather than 8,161. The whole
library is 707 KB on disk beside the binary.
...
iyi's own 707 KB prelude, the 1,299 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 17,219
...
- **iyi's own library is 17,219 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 17,219 lines. `--crystal` swaps it for Crystal's |
