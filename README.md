...
it uses, and iyi's own library is 16,146 lines rather than 8,161. The whole
library is 642 KB on disk beside the binary.
...
iyi's own 642 KB prelude, the 1,281 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 16,146
...
- **iyi's own library is 16,146 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 16,146 lines. `--crystal` swaps it for Crystal's |
