...
for phrase in "== request" "== response"; do
...
old = '{rest[0, sp2].to_i, rest[sp2 + 1, rest.bytesize - sp2 - 1]}'
if old not in src:
    raise SystemExit("patch site missing")
Path("$WORK/patched/std/http.iyi").write_text(src.replace(old, '{0, rest}', 1))
PY
if [ $? -ne 0 ]; then
