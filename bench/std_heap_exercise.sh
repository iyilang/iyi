...
# Exercises `std/heap`.
#
#     bash bench/std_heap_exercise.sh
set -u
...
build_and_run() {
  ...
  shift 2
  if ! "$IYI" build "$@" -o "$WORK/$name" "$REPO/bench/std_heap_exercise.iyi" \
...
echo "== the std/heap exercise, plain build"
build_and_run "plain" heap-plain
if ! grep -q "ALL CHECKS PASSED" "$WORK/heap-plain.out" 2>/dev/null; then
  echo "plain: missing pass sentinel"
  status=1
fi

echo
echo "== every heap section reported"
for phrase in "== order"; do
  if ! grep -q "$phrase" "$WORK/heap-plain.out" 2>/dev/null; then
...
build_and_run "release" heap-release --release >/dev/null
if ! grep -q "ALL CHECKS PASSED" "$WORK/heap-release.out" 2>/dev/null; then
...
PY
  if [ $? -ne 0 ]; then
    ...
    status=1
  elif IYI_PATH="$WORK/patched${PSEP}$REPO/src${PSEP}$REPO/samples/iyi" "$IYI" run "$REPO/bench/std_heap_exercise.iyi" >"$WORK/mut.out" 2>&1; then
    echo "  the exercise PASSED on a broken module"
    status=1
  else
    echo "  a broken heap is caught"
  fi
fi

echo
if [ "$status" -eq 0 ]; then
  echo "the std/heap exercise holds"
else
  echo "the std/heap exercise did not hold"
fi
exit $status
