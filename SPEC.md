...
| iyi's own prelude | 15,805 lines, of which 3,258 are the library held to the 3,734 ceiling (4,657 with every platform's floor, which the ceiling stopped counting after Windows); the rest is the collector, the scheduler and the float printer, which 0.1.0's prelude got from libgc, pthreads and libc |
...
> "No standard library worth the name" is still true of iyi's own 15,805 lines
...
**3. A deliberately tiny prelude, written in iyi. Done: 15,805 lines,
primitives included, of which the library is 3,258.** Not a standard library:
...
**3,258 lines** of the 15,805, measured by `bench/doc_numbers.py` as
everything under `src/iyi/` except those three and except every platform's
floor of 1,399 lines — the arms behind `flag?(:win32)`, `flag?(:linux)`,
`flag?(:darwin)` and `flag?(:wasm32)`, which the paragraph on the breach
below settles and explains. Opening `src/iyi/` counts 4,657 with the floor
...
**And it was breached again by Windows' own floor — 4,657 of 3,734, 923
...
where most of the 923 lines are.
...
exists for the same reason — the platform floor measures **1,399 lines**
of the 4,657, and Windows is the largest arm of it by a wide margin, which
...
The rewrite means the library gives back 923 lines, which at this size
...
and the library is **3,258** of 3,734 with 476 to spare.
...
| Library | 8,161 lines (3,551 of it core) | 15,805-line own prelude + 38,983 in std |
...
12. ~~**Using Crystal code.**~~ **Specified in III.6; the direct source mode
    ...
    **The Crystal ecosystem, and what a shard would cost.** Ten thousand
    ...
    Crystal's 8,161-line standard library against iyi's own 15,805-line prelude.
