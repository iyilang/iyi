...
it uses, and iyi's own library is 15,994 lines rather than 8,161. The whole
library is 637 KB on disk beside the binary.
...
iyi's own 637 KB prelude, the 1,278 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 15,994
...
- **iyi's own library is 15,994 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 15,994 lines. `--crystal` swaps it for Crystal's |
