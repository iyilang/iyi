...
# beside `bin/iyi` is 600 KB rather than a standard library.
.PHONY: install_iyi
...
# ships only iyi's own 600 KB answers `require "json"` with "can't find file",
...
# Crystal's library requires `std/...`, the directory's 1,231 KB is a second
...
# Its prelude is its own, and it is 600 KB: `iyi` installed as `bin/iyi` finds
