...
| iyi's own prelude | 17,340 lines, of which 3,494 are the library held to the 3,734 ceiling (5,015 with every platform's floor, which the ceiling stopped counting after Windows); the rest is the collector, the scheduler and the float printer, which 0.1.0's prelude got from libgc, pthreads and libc |
| compiler | 115,435 lines, none of it written in iyi |
...
> "No standard library worth the name" is still true of iyi's own 17,340 lines
...
**3. A deliberately tiny prelude, written in iyi. Done: 17,340 lines,
primitives included, of which the library is 3,494.** Not a standard library:
...
**3,494 lines** of the 17,340, measured by `bench/doc_numbers.py` as
everything under `src/iyi/` except those three and except every platform's
floor of 1,521 lines — the arms behind `flag?(:win32)`, `flag?(:linux)`,
`flag?(:darwin)` and `flag?(:wasm32)`, which the paragraph on the breach
below settles and explains. Opening `src/iyi/` counts 5,015 with the floor
...
exists for the same reason — the platform floor measures **1,521 lines**
...
|---|---|---|
| Compiler | 24,984 lines, **written in Crystal** | 115,435 lines, Crystal, forked |
| Library | 8,161 lines (3,551 of it core) | 17,340-line own prelude + 39,966 in std |
...
12. ~~**Using Crystal code.**~~ **Specified in III.6; the direct source mode
    ...
    **The Crystal ecosystem, and what a shard would cost.** Ten thousand
    ...
    Crystal's 8,161-line standard library against iyi's own 17,340-line prelude.
