...
it uses, and iyi's own library is 16,207 lines rather than 8,161. The whole
library is 644 KB on disk beside the binary.
...
iyi's own 644 KB prelude, the 1,299 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 16,207
...
- **iyi's own library is 16,207 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 16,207 lines. `--crystal` swaps it for Crystal's |
