...
it uses, and iyi's own library is 17,472 lines rather than 8,161. The whole
library is 719 KB on disk beside the binary.
...
iyi's own 719 KB prelude, the 1,337 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 17,472
...
- **iyi's own library is 17,472 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 17,472 lines. `--crystal` swaps it for Crystal's |
