...
it uses, and iyi's own library is 16,192 lines rather than 8,161. The whole
library is 643 KB on disk beside the binary.
...
iyi's own 643 KB prelude, the 1,283 KB of `src/std` that `import std/...`
...
program *has*: 8,161 lines of Crystal's standard library instead of 16,192
...
- **iyi's own library is 16,192 lines, and its IO is `puts`, `print`, the
...
| [`src/iyi`](src/iyi) | iyi's own library, 16,192 lines. `--crystal` swaps it for Crystal's |
