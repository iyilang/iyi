#!/usr/bin/env bash
# Exercises `std/math`: sqrt, sin/cos, huge-arg reduction that does not panic.
#
...
old = 'return {0.0, 1.0} if q_f.abs >= 9223372036854775808.0'
if old not in src:
    raise SystemExit("patch site missing")
Path("$WORK/patched/std/math.iyi").write_text(src.replace(old, '# no huge-arg guard', 1))
PY
if [ $? -ne 0 ]; then
